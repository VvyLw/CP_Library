...
fn (p Pair[T,U]) rotate(ang int) Pair[f64, f64] {
	assert scope(0,ang,359)
	rad:=ma.radians(ang)
	return Pair[f64, f64]{p.first*ma.cos(rad)-p.second*ma.sin(rad), p.first*ma.sin(rad)+p.second*ma.cos(rad)}
}
fn (p Pair[T,U]) rtt90() Pair[U,T] { return Pair[U,T]{-p.second,p.first} }
fn (p Pair[T,U]) dot(q Pair[T,U]) T { return p.first*q.first+p.second*q.second }
fn (p Pair[T,U]) square() T { return p.dot(p) }
fn (p Pair[T,U]) grad() f64 { assert p.first!=0 return f64(p.second)/p.first }
fn (p Pair[T,U]) abs() f64 { return ma.hypot(p.first,p.second) }
